...
/* Client-side daily rate limit for the coach (localStorage, resets when the date changes). */
const coachToday = () => new Date().toISOString().slice(0,10);
function getCoachUsage(){
  try{
    const u = JSON.parse(localStorage.getItem("wc26-coach-usage")||"{}");
    if(u.date !== coachToday()) return { date: coachToday(), count: 0 };
    return { date: u.date, count: u.count||0 };
  }catch(e){ return { date: coachToday(), count: 0 }; }
}
function bumpCoachUsage(){
  const u = getCoachUsage();
  const next = { date: u.date, count: u.count+1 };
  try{ localStorage.setItem("wc26-coach-usage", JSON.stringify(next)); }catch(e){}
  return next;
}
...
function Coach({players, sq, mySquad, cap, msgs, setMsgs, onApply}) {
  ...
  const [used,setUsed]=useState(()=>getCoachUsage().count);
  const ref=useRef(null);
  useEffect(()=>{ ref.current?.scrollTo(0,99999); },[msgs,busy]);
  const left=Math.max(0, COACH_DAILY_LIMIT - used);
  const ask=async()=>{
    const text=inp.trim(); if(!text||busy||left<=0) return;
    const hist=[...msgs,{role:"user",content:text}];
    setMsgs(hist); setInp(""); setBusy(true);
    try{
      let out;
      if(COACH_MODE==="gemini"){
        // Production: Vercel function proxy that holds the key and rate-limits server-side too.
        ...
        out=d.text||d.error||"Try again.";
      }else{
        // "claude": direct Anthropic call — personal artifact build only, unchanged.
  ...
  return <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 175px)"}}>
    ...
    <div style={{padding:"6px 12px 0",fontSize:12,textAlign:"center",color:left>0?"var(--dim)":"var(--amber)"}}>
      {left>0
        ? `${left}/${COACH_DAILY_LIMIT} questions left today`
        : "You've used your 5 free coach questions today. Come back tomorrow! ☕"}
    </div>
    <div className="searchbar" style={{paddingTop:8}}>
      <input placeholder={left>0?'e.g. "Draft me a squad" or "Best MD1 captain?"':"Daily limit reached — see you tomorrow"}
        value={inp} onChange={e=>setInp(e.target.value)} disabled={busy||left<=0}
        onKeyDown={e=>e.key==="Enter"&&ask()}/>
      <button className="btn g" onClick={ask} disabled={busy||left<=0}>Ask</button>
    </div>
  </div>;
}
